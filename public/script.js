...
let test = async () => {
  ...
  if(!selectedTweet.id || replyField.value == '') {
    return ;
  }
  iconLoaded();
  console.log(replyField.value,selectedTweet.id,selectedTweet.user.screen_name);
  let data = {
    data: replyField.value,
    id: selectedTweet.id_str,
    name: selectedTweet.user.screen_name
  }
  replyField.value = '';
...
let insertSelection = (data) => {
  ...
  const text1 = `<table style="width:90%; margin-top: 10px;">
    <tr class = "chat-loader-profile">
      <td style="text-align: right; font-weight: 100;"><img style="width: 40px; height: 40px;" src=${data[0].user.profile_image_url} class="card-img"></td>
      <td style="text-align: center; font-weight: 500;">${data[0].user.name}</td>
      <td><span class="dot"></span></td>
    </tr>
  </table>`;
  const text2 = `<img class="profile-image" src="${data[0].user.profile_image_url}" alt="Smiley face">
  ...
  data.forEach(ele => {
    let time = ele.created_at.split(" ");
    time = time[3];
    const text3 = `<table style="width: 90%; margin-bottom: 20px;">
      <tr>
        <td style="width: 60px;">
          <img style="width: 25px; height: 25px; float: right;" src=${ele.user.profile_image_url} class="card-img">
        </td>
        <td style="padding-left: 20px; font-weight: 500;">
        ${ele.user.name}
        </td>
...
const renderList = (items) => {
  items.forEach(ele => {
    if(ele.in_reply_to_status_id_str === null) {
